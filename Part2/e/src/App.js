...
const Details = ({value,persons,setPersons}) => {
  // console.log(...value)
  return (
    <div>
      {value.map(val => <Contact key={val.id} naam={val.name} number={val.number} id={val.id} setPersons={setPersons} ></Contact>)}
    </div>
  )
}
...
const App = () => {
  ...
  return (
    <div>
      ...
      <h2>Numbers</h2>
      <div>
      <Details value={val} persons={persons} setPersons={setPersons}>
      </Details>
